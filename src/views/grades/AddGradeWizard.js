...
import { create } from 'src/services/grade.service'
import { toast } from 'react-hot-toast'
import { getBySchoolId } from 'src/services/teacher.service'

const steps = ['Select School', 'Select Teacher', 'Add Grade']
...
const AddTeacherWizard = ({ onComplete }) => {
  ...
  const getComponent = step => {
    switch (step) {
      ...
      case 2:
        return <AddGrade back={handleBack} finish={finish} schoolId={selectedSchool} teacherId={selectedTeacher} />
      default:
        return <></>
    }
  }
    ...
    return (
      <Card
        sx={{
          ...
          border: theme =>
            selected === school.id ? `1px solid ${theme.palette.primary.main}` : `1px solid transparent`,
          '&:hover': {
            boxShadow: 5
          }
        }}
...
const AddGrade = ({ back, finish, schoolId, teacherId }) => {
  ...
  const addTeacher = async () => {
    // add grade
    const result = await create(name, startTime, offTime, auth.user.id, schoolId, teacherId)
  ...
  return (
    <>
      ...
      <Grid container spacing={6}>
        ...
        <Grid item xs={6}>
          <Stack>
            <Typography variant='label'>Off Time</Typography>
            <TextField type='time' value={offTime} onChange={e => setOffTime(e.target.value)} fullWidth size='small' />
          </Stack>
        </Grid>
      </Grid>
