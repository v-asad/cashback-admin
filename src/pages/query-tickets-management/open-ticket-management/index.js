...
const OpenTicketManage = () => {
      ...
      render: (_, object, index) => (
        <>
         <Grid spacing={0}>
            ...
            <Link href='javascript:void(0)' onClick={() => editItem(object.id)} sx={{ mr: 5,display:'flex'}}>Edit</Link>
  ...
  const updateTicket = () => {
    ...
    if(!errors){
      setOpen(true)
      axios
      .put(`${process.env.NEXT_PUBLIC_API_URL}/controlpanel/tickets/${ticket.id}`, {
                ...
                <CardContent>
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12}>
                      <Box sx={{ minWidth: 120 }}>
                        <FormControl fullWidth>
                          <TextareaAutosize xs={6} fullWidth minRows={3} label='Response' placeholder='Response' onChange={e => {setTicketResponse(e.target.value)}} value={ticketResponse} />
                        </FormControl>
                      </Box>
                    </Grid>
                  </Grid>
